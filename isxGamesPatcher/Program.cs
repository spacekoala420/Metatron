...
namespace isxGamesPatcher
{
	...
	{
		// Modern HttpClient for downloading updates
		...
		public bool DownloadFile(FileInfoClass FileInfo)
		{
			...
			try
			{
				// Configure HttpClient request with custom headers
				var requestMessage = new HttpRequestMessage(HttpMethod.Get, FileInfo.Url);
				requestMessage.Headers.Add("Pragma", "no-cache");
				requestMessage.Headers.Add("cache-control", "no-cache");
				requestMessage.Headers.Add("User-Agent", "isxGamesPatcher/" + isxGamesPatcher.Properties.Resources.VERSION);
				requestMessage.Headers.IfModifiedSince = DateTime.Now.AddYears(-1);

				// Use synchronous call to maintain existing threading model
				...
				if (response != null)
					{
						...
						RStream = response.Content.ReadAsStreamAsync().GetAwaiter().GetResult();
			...
			{
				ErrorStr = "HTTP Error: " + e.Message + " - " + FileInfo.Url;
			...
			{
				ErrorStr = "HTTP Timeout: " + FileInfo.Url;
				InnerSpace.Echo("isxGamesPatcher: " + "   " + ErrorStr);
				Downloading = false;
				return false;
			}
	...
	{
		// Modern HttpClient for downloading manifests
		private static readonly HttpClient _httpClient = new HttpClient
		{
			Timeout = TimeSpan.FromSeconds(60)
		};
		...
		public bool ParseManifest()
		{
			try
			{
				...
				// Configure HttpClient request with custom headers
				var requestMessage = new HttpRequestMessage(HttpMethod.Get, Project.ManifestURL);
				requestMessage.Headers.Add("Pragma", "no-cache");
				requestMessage.Headers.Add("cache-control", "no-cache");
				requestMessage.Headers.Add("User-Agent", "isxGamesPatcher/" + isxGamesPatcher.Properties.Resources.VERSION);
				requestMessage.Headers.IfModifiedSince = DateTime.Now.AddYears(-1);
