...
namespace Metatron.BehaviorModules
{
	internal class Ratting : BehaviorBase
	{
		...
		/// <summary>
		/// Indicates whether we're actively engaged in combat (killing rats).
		/// When true, propulsion modules should remain active to maintain transversal velocity.
		/// </summary>
		public bool IsInActiveCombat { get; private set; }

	    private int PulsesToWaitForRats = 7; // For asteroid belts
		private int PulsesToWaitForAnomalyWave = 7; // For anomaly waves
		...
		// Anomaly blacklist tracking for non-fleet player occupation
		private readonly Dictionary<long, DateTime> _anomaliesOccupiedByNonFleetPlayers = new Dictionary<long, DateTime>();
		private const int ANOMALY_BLACKLIST_MINUTES = 20;
		...
		public override void Pulse()
		{
			...
			if (_meCache.InSpace)
			{
				...
				// Clean up expired anomaly blacklists
				CleanupExpiredAnomalyBlacklists();
			}
	    ...
	    private void GetEntities()
	    {
	        ...
	        _ratNpcEntities.AddRange(
	            _entityProvider.EntityWrappers.Where(
		...
		private void ChangeBelt()
		{
			...
			// Only apply random wait when leaving a completed site (wrecks present, no rats)
			// Skip wait when: undocking, fleeing, or at an incomplete site
			var hasWrecks = _meCache.InSpace && _entityProvider.EntityWrappers.Any(e => e.GroupID == (int)GroupIDs.Wreck);
			if (hasWrecks && !_ratNpcEntities.Any() && !_meCache.InStation)
			{
			...
			_doWarpInCheck = true;
			_warpInCheckCounter = _rattingConfiguration.IsAnomalyMode ? PulsesToWaitForAnomalyWave : PulsesToWaitForRats;
                ...
                case ChangeAnomalyStates.SelectAnomaly:
                    {
                        ...
                        {
							var allAnomalies = _anomalyProvider.GetAnomalies();
							var selectedAnomalyTypes = _rattingConfiguration.StatusByAnomalyType
								.Where(p => p.Second)
								.Select(p => p.First)
								.ToList();

							var matchingAnomalies = allAnomalies
								.Where(a => selectedAnomalyTypes.Any(type => a.DungeonName.Contains(type)))
								.ToList();

							var blacklistedCount = matchingAnomalies
								.Count(a => IsAnomalyBlacklistedByNonFleetPlayer(a.ID));

							LogMessage(methodName, LogSeverityTypes.Critical,
								"CRITICAL: No anomalies available to run! " +
								"Total matching anomalies: {0}, Blacklisted by non-fleet players: {1}, Claimed by others: {2}",
								matchingAnomalies.Count,
								blacklistedCount,
								matchingAnomalies.Count(a => _anomalyClaimTracker.IsAnomalyClaimedByOther(a.ID)));

							if (matchingAnomalies.Count > 0 && blacklistedCount == matchingAnomalies.Count)
							{
								LogMessage(methodName, LogSeverityTypes.Critical,
									"ALL anomalies are occupied by non-fleet players! " +
									"Going to safe spot to avoid obvious bot behavior (warping back and forth). " +
									"Will retry when blacklists expire in {0} minutes.",
									ANOMALY_BLACKLIST_MINUTES);
							}
							else
							{
								LogMessage(methodName, LogSeverityTypes.Standard,
									"Error: Could not find an anomaly matching selected anomalies.");
							}
                        ...
                        // DO NOT claim anomaly for ourselves - fleet members should cooperate!
                        // Only claim anomalies on behalf of non-fleet players (done in CheckPcs)
                ...
                case ChangeAnomalyStates.ValidateAnomaly:
                    {
                        ...
                        // REMOVED: IsAnomalyClaimedByOther check
                        // Fleet members should NOT avoid anomalies claimed by other fleet members
                        // Only avoid anomalies claimed on behalf of non-fleet players (checked in GetAvailableAnomalies)
	    ...
	    {
			var methodName = "SelectAnomaly";
			LogTrace(methodName);

	        var anomalies = GetAvailableAnomalies();

			// Filter out anomalies blacklisted by non-fleet players
			var availableAnomalies = anomalies
				.Where(a => !IsAnomalyBlacklistedByNonFleetPlayer(a.ID))
				.ToList();

			if (availableAnomalies.Count == 0 && anomalies.Any())
			{
				// All available anomalies are blacklisted!
				LogMessage(methodName, LogSeverityTypes.Critical,
					"All {0} available anomalies are occupied by non-fleet players. No anomalies to run!",
					anomalies.Count);
				return null;
			}

	        //Grab the first one I'm not at.
	        var nearbyAnomaly = availableAnomalies.FirstOrDefault(b => _meCache.ToEntity.DistanceTo(b.X, b.Y, b.Z) < (int) Ranges.Warp*2);
	        var anomaly = availableAnomalies.FirstOrDefault(b => (nearbyAnomaly == null || b.ID != nearbyAnomaly.ID) &&
	                                                    _meCache.ToEntity.DistanceTo(b.X, b.Y, b.Z) >= (int) Ranges.Warp*2);

			if (anomaly == null)
			{
				LogMessage(methodName, LogSeverityTypes.Standard,
					"No suitable anomalies found (total available: {0}, after blacklist: {1})",
					anomalies.Count, availableAnomalies.Count);
			}
	    ...
	    {
			var methodName = "BookmarkAnomalyForSalvage";
			LogTrace(methodName);
			...
			var currentX = _meCache.ToEntity.X;
			var currentY = _meCache.ToEntity.Y;
			var currentZ = _meCache.ToEntity.Z;

			// Get all salvage bookmarks in current system
			var salvageBookmarks = Core.Metatron.BookMarkCache
				.GetBookMarksStartingWith(_movementConfiguration.SalvagingPrefix, true);

			// Find bookmarks within 250km (same anomaly grid)
			const double BOOKMARK_PROXIMITY_THRESHOLD = 250000; // 250km
			var nearbyBookmarks = salvageBookmarks
				.Where(bm => DistanceTo(bm.X, bm.Y, bm.Z, currentX, currentY, currentZ) < BOOKMARK_PROXIMITY_THRESHOLD)
				.OrderBy(bm => bm.Id) // Order by ID (oldest first)
				.ToList();

			if (nearbyBookmarks.Count > 0)
			{
				// Bookmarks already exist at this location
				LogMessage(methodName, LogSeverityTypes.Debug,
					"Found {0} existing salvage bookmark(s) at this location", nearbyBookmarks.Count);

				if (nearbyBookmarks.Count > 1)
				{
					// Multiple bookmarks detected - delete extras!
					var bookmarkToKeep = nearbyBookmarks.First();
					var bookmarksToDelete = nearbyBookmarks.Skip(1).ToList();

					LogMessage(methodName, LogSeverityTypes.Standard,
						"Multiple salvage bookmarks detected! Keeping bookmark \"{0}\" ({1}), deleting {2} duplicate(s)",
						bookmarkToKeep.Label, bookmarkToKeep.Id, bookmarksToDelete.Count);

					foreach (var bookmark in bookmarksToDelete)
					{
						LogMessage(methodName, LogSeverityTypes.Debug,
							"Deleting duplicate bookmark \"{0}\" ({1})", bookmark.Label, bookmark.Id);
						Core.Metatron.BookMarkCache.RemoveCachedBookMark(bookmark);
					}
				}
				else
				{
					LogMessage(methodName, LogSeverityTypes.Debug,
						"Salvage bookmark already exists: \"{0}\" ({1})",
						nearbyBookmarks[0].Label, nearbyBookmarks[0].Id);
				}

				// Bookmark already exists, don't create new one
				return;
			}

			// No bookmark exists nearby - create new one
			var wreck = _entityProvider.EntityWrappers
		...
		{
			LogMessage(methodName, LogSeverityTypes.Debug,
				"No wrecks found - cannot create salvage bookmark");
			return;
		}

			LogMessage(methodName, LogSeverityTypes.Standard,
				"Creating salvage bookmark at anomaly location");
			_bookmarks.CreateSalvagingBookmark(wreck);

			// RACE CONDITION PROTECTION: Check again after creation
			// Another instance may have created a bookmark at the same time
			// Give the bookmark cache a moment to update
			System.Threading.Thread.Sleep(500); // 500ms delay

			// Re-fetch salvage bookmarks
			salvageBookmarks = Core.Metatron.BookMarkCache
				.GetBookMarksStartingWith(_movementConfiguration.SalvagingPrefix, true);

			nearbyBookmarks = salvageBookmarks
				.Where(bm => DistanceTo(bm.X, bm.Y, bm.Z, currentX, currentY, currentZ) < BOOKMARK_PROXIMITY_THRESHOLD)
				.OrderBy(bm => bm.Id)
				.ToList();
			...
			if (nearbyBookmarks.Count > 1)
			{
				// Race condition occurred! Multiple instances created bookmarks simultaneously
				LogMessage(methodName, LogSeverityTypes.Standard,
					"Race condition detected: {0} bookmarks created simultaneously. Cleaning up duplicates",
					nearbyBookmarks.Count);

				// Find the bookmark we just created (likely the newest/highest ID)
				var ourBookmark = nearbyBookmarks
					.OrderByDescending(bm => bm.Id)
					.FirstOrDefault(bm => bm.CreatorId == _meCache.Me.ID);

				var bookmarkToKeep = nearbyBookmarks.First(); // Keep oldest

				// If our bookmark is not the oldest, delete it
				if (ourBookmark != null && ourBookmark.Id != bookmarkToKeep.Id)
				{
					LogMessage(methodName, LogSeverityTypes.Debug,
						"Deleting our newly created bookmark \"{0}\" ({1}) in favor of older bookmark \"{2}\" ({3})",
						ourBookmark.Label, ourBookmark.Id, bookmarkToKeep.Label, bookmarkToKeep.Id);
					Core.Metatron.BookMarkCache.RemoveCachedBookMark(ourBookmark);
				}

				// Also delete any other duplicates
				var otherDuplicates = nearbyBookmarks
					.Where(bm => bm.Id != bookmarkToKeep.Id && (ourBookmark == null || bm.Id != ourBookmark.Id))
					.ToList();

				foreach (var duplicate in otherDuplicates)
				{
					LogMessage(methodName, LogSeverityTypes.Debug,
						"Deleting duplicate bookmark \"{0}\" ({1})", duplicate.Label, duplicate.Id);
					Core.Metatron.BookMarkCache.RemoveCachedBookMark(duplicate);
				}
			}
	    }

		/// <summary>
		/// Helper method for calculating distance between two 3D points
		/// </summary>
		...
		/// <summary>
		/// Track an anomaly as being occupied by a non-fleet player.
		/// This anomaly will be excluded from selection for 40 minutes.
		/// </summary>
		private void TrackAnomalyOccupiedByNonFleetPlayer(long anomalyId)
		{
			var methodName = "TrackAnomalyOccupiedByNonFleetPlayer";
			LogTrace(methodName, "AnomalyId: {0}", anomalyId);

			lock (_anomaliesOccupiedByNonFleetPlayers)
			{
				if (_anomaliesOccupiedByNonFleetPlayers.ContainsKey(anomalyId))
				{
					// Update timestamp
					_anomaliesOccupiedByNonFleetPlayers[anomalyId] = DateTime.Now;
					LogMessage(methodName, LogSeverityTypes.Debug,
						"Updated non-fleet player occupation timestamp for anomaly {0}", anomalyId);
				}
				else
				{
					// Add new entry
					_anomaliesOccupiedByNonFleetPlayers.Add(anomalyId, DateTime.Now);
					LogMessage(methodName, LogSeverityTypes.Standard,
						"Anomaly {0} marked as occupied by non-fleet player, blacklisted for {1} minutes",
						anomalyId, ANOMALY_BLACKLIST_MINUTES);
				}
			}
		}

		/// <summary>
		/// Check if an anomaly is currently blacklisted due to non-fleet player occupation.
		/// </summary>
		private bool IsAnomalyBlacklistedByNonFleetPlayer(long anomalyId)
		{
			var methodName = "IsAnomalyBlacklistedByNonFleetPlayer";
			LogTrace(methodName, "AnomalyId: {0}", anomalyId);
			...
			lock (_anomaliesOccupiedByNonFleetPlayers)
			{
				...
				var blacklistTime = _anomaliesOccupiedByNonFleetPlayers[anomalyId];
				var blacklistExpiry = blacklistTime.AddMinutes(ANOMALY_BLACKLIST_MINUTES);

				if (DateTime.Now < blacklistExpiry)
				{
					var remainingMinutes = (blacklistExpiry - DateTime.Now).TotalMinutes;
					LogTrace(methodName,
						"Anomaly {0} is blacklisted (expires in {1:F1} minutes)",
						anomalyId, remainingMinutes);
					return true;
				}

				// Blacklist expired - remove it
				LogMessage(methodName, LogSeverityTypes.Debug,
					"Blacklist expired for anomaly {0}", anomalyId);
				_anomaliesOccupiedByNonFleetPlayers.Remove(anomalyId);
				return false;
			}
		}

		/// <summary>
		/// Clean up expired anomaly blacklist entries.
		/// Called during Pulse() to prevent dictionary from growing indefinitely.
		/// </summary>
		private void CleanupExpiredAnomalyBlacklists()
		{
			var methodName = "CleanupExpiredAnomalyBlacklists";
			LogTrace(methodName);

			lock (_anomaliesOccupiedByNonFleetPlayers)
			{
				var expiredIds = _anomaliesOccupiedByNonFleetPlayers
					.Where(kvp => DateTime.Now >= kvp.Value.AddMinutes(ANOMALY_BLACKLIST_MINUTES))
					.Select(kvp => kvp.Key)
					.ToList();
				...
				{
					LogMessage(methodName, LogSeverityTypes.Debug,
						"Removing expired blacklist for anomaly {0}", id);
					_anomaliesOccupiedByNonFleetPlayers.Remove(id);
				}
			}
		}
		...
		private bool CheckNpcs()
		{
			...
			if (_doWarpInCheck)
			{
				//If nothing's here delay a bit for warp-ins / next wave spawns
				if (--_warpInCheckCounter >= 0)
				{
					if (_rattingConfiguration.IsAnomalyMode)
					{
						LogMessage(methodName, LogSeverityTypes.Debug, "Waiting for next wave to spawn (pulse {0}/{1})...",
							(PulsesToWaitForAnomalyWave - _warpInCheckCounter), PulsesToWaitForAnomalyWave);
					}
					else if (_asteroidBelts.IsAtAsteroidBelt())
					{
            ...
            else if (_rattingConfiguration.IsAnomalyMode)
            {
                // NPCs were present earlier but now all dead
                // Reset the warp-in check to wait for next wave
                LogMessage(methodName, LogSeverityTypes.Debug,
                    "All rats cleared, resetting warp-in check to wait for next wave...");
                _doWarpInCheck = true;
                _warpInCheckCounter = PulsesToWaitForAnomalyWave;
                return true;  // Keep waiting for next wave
            }
		...
		private bool CheckPcs()
		{
		    ...
		    {
				// Get fleet member names
				var fleetMemberNames = _meCache.FleetMembers
					.Select(fm => fm.Name)
					.ToList();

				// Separate fleet members from non-fleet players
				var nonFleetPlayers = _ratPcEntities
					.Where(e => !fleetMemberNames.Contains(e.Name))
					.ToList();

				var fleetMembers = _ratPcEntities
					.Where(e => fleetMemberNames.Contains(e.Name))
					.ToList();

				// Log both groups
				if (fleetMembers.Count > 0)
				{
					LogMessage(methodName, LogSeverityTypes.Debug,
						"Fleet members detected ({0}): {1}",
						fleetMembers.Count,
						string.Join(", ", fleetMembers.Select(e => e.Name)));
				}

				if (nonFleetPlayers.Count > 0)
				{
					LogMessage(methodName, LogSeverityTypes.Debug,
						"Non-fleet players detected ({0}): {1}",
						nonFleetPlayers.Count,
						string.Join(", ", nonFleetPlayers.Select(e => e.Name)));

					// Check standings via Social module (handles all standing checks including Corps/Alliances)
					if (!_social.IsLocalSafe)
					{
						LogMessage(methodName, LogSeverityTypes.Critical,
							"Non-fleet player(s) failed standings check - local is NOT safe");

						// Claim anomaly on behalf of non-fleet player
						if (_rattingConfiguration.IsAnomalyMode)
						{
							...
							var nearestNonFleetPlayer = nonFleetPlayers
								.OrderBy(e => e.Distance)
								.FirstOrDefault();

							if (anomaly != null && nearestNonFleetPlayer != null)
							{
								var distanceToPlayer = _meCache.ToEntity.DistanceTo(nearestNonFleetPlayer.ID);
								if (distanceToPlayer < 2 * (int)Ranges.Warp)
								{
									LogMessage(methodName, LogSeverityTypes.Debug,
										"Claiming anomaly {0} ({1}) for non-fleet player {2}",
										anomaly.Name, anomaly.ID, nearestNonFleetPlayer.Name);

									_anomalyClaimTracker.ClaimAnomaly(anomaly.ID, nearestNonFleetPlayer.ID);

									// Track this anomaly as occupied by non-fleet player
									TrackAnomalyOccupiedByNonFleetPlayer(anomaly.ID);
								}
							}
						}

						return false; // WARP AWAY from non-fleet players with bad standings
					}
					else
					{
						// Non-fleet players present, but standings are good - stay and cooperate
						LogMessage(methodName, LogSeverityTypes.Debug,
							"Non-fleet player(s) present but standings check PASSED - local is safe, staying");
						return true;
					}
				}

				// Only fleet members present - stay and cooperate!
				LogMessage(methodName, LogSeverityTypes.Debug,
					"Only fleet members present - staying to cooperate");
				return true;
		    }
