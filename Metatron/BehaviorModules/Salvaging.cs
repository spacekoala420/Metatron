...
namespace Metatron.BehaviorModules
{
    internal class Salvaging : BehaviorBase
    {
        ...
        private readonly List<IEntityWrapper> _cargoContainerEntities = new List<IEntityWrapper>();
        ...
        private void GetEntities()
        {
            ...
            // Add WRECKS to the salvage list - these can be salvaged and looted
            _wreckNpcEntities.AddRange(
                Core.Metatron.EntityProvider.EntityWrappers.Where(
                    entity => entity.GroupID == (int)GroupIDs.Wreck && entity.Distance < (int)Ranges.Warp && entity.HaveLootRights));

            // Add CARGO CONTAINERS - these can only be looted, not salvaged
            _cargoContainerEntities.AddRange(
                Core.Metatron.EntityProvider.EntityWrappers.Where(
                    entity => entity.GroupID == (int)GroupIDs.CargoContainer && entity.Distance < (int)Ranges.Warp));
        }

        private void ClearEntities()
        {
            _wreckNpcEntities.Clear();
            _cargoContainerEntities.Clear();
        }
        ...
        private void SalvageWrecks()
        {
            ...
            // Combine both wrecks and cargo containers for processing
            var allLootables = _wreckNpcEntities.Concat(_cargoContainerEntities).ToList();

            if (allLootables.Count == 0)
            {
                LogMessage(methodName, LogSeverityTypes.Debug, "No wrecks or cargo containers detected.");
                return;
            }

            var closestEntity = allLootables.OrderBy(e => e.Distance).First();

            if (closestEntity.Distance > _ship.MaxTargetRange)
            {
                LogMessage(methodName, LogSeverityTypes.Debug, "All lootables are out of range, moving closer...");
            ...
            {
                // Queue wrecks (can be salvaged and looted)
                if (_wreckNpcEntities.Count > 0)
                {
                    QueueEntities(_wreckNpcEntities);
                }

                // Queue cargo containers (can only be looted, not salvaged)
                if (_cargoContainerEntities.Count > 0)
                {
                    QueueEntities(_cargoContainerEntities);
                }
            }
        }
        ...
        /// <summary>
        /// Determine if there are lootable entities (wrecks or cargo containers) at the site
        /// Site is only considered "complete" when BOTH wrecks and containers are gone
        /// </summary>
        private bool CheckWrecks()
        {
            ...
            // Site has content if there are wrecks OR cargo containers
            if (_wreckNpcEntities.Count > 0 || _cargoContainerEntities.Count > 0)
            {
                LogTrace(methodName, "Found {0} wrecks and {1} cargo containers",
                    _wreckNpcEntities.Count, _cargoContainerEntities.Count);
                return true;
            }
            else
            {
                LogTrace(methodName, "No wrecks or cargo containers - site is complete");
                return false;
            }
        }
    }
