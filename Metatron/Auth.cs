...
namespace Metatron
{
    ...
    {
        // Modern HttpClient for auth server requests
        ...
        private __err_retn Authenticate(string authServerUrl, string key, string iv)
        {
			try
			{
				...
				try
				{
					// Use synchronous HTTP call with modern HttpClient
					var response = _httpClient.GetAsync(authServerUrl).GetAwaiter().GetResult();
					response.EnsureSuccessStatusCode();
					base64AuthString = response.Content.ReadAsStringAsync().GetAwaiter().GetResult();
				}
				catch (HttpRequestException e)
				{
					_logging.LogException("Authentication", e, "Authenticate", "Caught exception during HTTP request: {0}", e.Message);
					return new __err_retn(false, ERROR_BAD_RESPONSE, false);
				}
				catch (System.Threading.Tasks.TaskCanceledException e)
				{
					_logging.LogException("Authentication", e, "Authenticate", "Request timeout: {0}", e.Message);
					return new __err_retn(false, ERROR_BAD_RESPONSE, false);
				}
				catch (Exception e)
				{
					_logging.LogException("Authentication", e, "Authenticate", "Caught exception reading response: {0}", e.Message);
					return new __err_retn(false, ERROR_BAD_RESPONSE, false);
				}
			...
			{
				_logging.LogException("Authentication", e, "Authenticate", "Caught exception:", e.Message);
				return new __err_retn(false, ERROR_BAD_RESPONSE, false);
			}
