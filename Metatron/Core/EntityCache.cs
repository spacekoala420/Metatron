    ...
    internal sealed class EntityCache : ModuleBase, IEntityCache
    {
        ...
        private void TryGetEntityInfo(object stateInfo)
        {
            ...
            try
            {
                ...
                var url = String.Format("https://esi.evetech.net/latest/universe/types/{0}/?datasource=tranquility", typeId);
                ...
                {
                    // Timeout occurred
                    LogMessage(methodName, LogSeverityTypes.Standard, "Request timeout for entity type {0}", typeId);
                    lock (this)
                    {
                        if (_entitiesDoingGetInfo.Contains(typeId))
                        {
