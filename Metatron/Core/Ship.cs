...
namespace Metatron.Core
{
    ...
    internal sealed class Ship : ModuleBase, IShip
    {
        ...
        {
            var methodName = "RepairIfNeeded";
            LogTrace(methodName);

            // Must be docked
            if (!_meCache.InStation)
            {
                LogMessage(methodName, LogSeverityTypes.Debug, "Not in station, cannot repair");
                return true;
            }

            // Open repair window if not open
            if (!IsRepairWindowOpen)
            {
                LogMessage(methodName, LogSeverityTypes.Debug, "Opening repair window to check for damage");
                _shipCache.Ship.ToItem.GetRepairQuote();
                return false; // Wait for window to open
            }

            // Check if repairs needed
            if (_eveWindowProvider.GetRepairShopWindow().TotalCost == "0")
            {
                LogMessage(methodName, LogSeverityTypes.Debug, "No repairs needed");
                return true; // Nothing to repair - done
            }

            // Handle repair confirmation modal
            if (_eveWindowProvider.GetRepairItemsModalWindow() != null)
            {
                LogMessage(methodName, LogSeverityTypes.Standard, "Confirming repairs");
            ...
            {
                LogMessage(methodName, LogSeverityTypes.Standard, "Repairing ship, modules, and drones");
