...
namespace Metatron.ActionModules
{
    ...
    public sealed class Offensive : ModuleBase, IOffensive
    {
        ...
        private void Orbit(IEntityWrapper activeTarget)
        {
            ...
            // Anchor wreck orbiting feature (if enabled)
            if (Core.Metatron.Config.RattingConfig.UseAnchorWreckOrbiting)
            {
                ...
                // CHECK 1: Already orbiting a wreck that still exists?
                // If yes, KEEP ORBITING IT regardless of current distance (ship needs time to establish orbit!)
                if (existingOrbitDestination != null &&
                    Core.Metatron.EntityProvider.EntityWrappersById.ContainsKey(existingOrbitDestination.EntityId))
                {
                    var currentOrbitTarget = Core.Metatron.EntityProvider.EntityWrappersById[existingOrbitDestination.EntityId];

                    // If orbiting a wreck that still exists, maintain orbit (don't switch!)
                    if (currentOrbitTarget.GroupID == (int)GroupIDs.Wreck)
                    {
                        LogTrace(methodName, "Already orbiting anchor wreck '{0}' ({1}) at {2:F0}m - maintaining orbit",
                            currentOrbitTarget.Name, currentOrbitTarget.ID, currentOrbitTarget.Distance);
                        return; // Keep current orbit, don't change anything
                    }
                }

                // Calculate tolerance for SELECTING new anchor: ±25% (15-25km for 20km orbit)
                var minOrbitDistance = distance * 0.75;
                var maxOrbitDistance = distance * 1.25;

                // CHECK 2: Find wrecks to orbit
                var nearbyWrecks = _entityProvider.EntityWrappers
                    .Where(e => e.GroupID == (int)GroupIDs.Wreck
                        && e.Distance < (int)Ranges.Warp
                        && e.HaveLootRights)
                    .ToList();

                if (nearbyWrecks.Any())
                {
                    // Prefer wreck already at orbit distance (15-25km), otherwise closest
                    var wrecksAtOrbitDistance = nearbyWrecks
                        .Where(w => w.Distance >= minOrbitDistance && w.Distance <= maxOrbitDistance)
                        .OrderBy(w => Math.Abs(w.Distance - distance))  // Closest to exact orbit distance
                        .ToList();

                    var selectedWreck = wrecksAtOrbitDistance.Any()
                        ? wrecksAtOrbitDistance.First()
                        : nearbyWrecks.OrderBy(w => w.Distance).First();

                    LogMessage(methodName, LogSeverityTypes.Debug,
                        "Orbiting anchor wreck '{0}' ({1}) at {2}m (current distance: {3:F0}m)",
                        selectedWreck.Name, selectedWreck.ID, distance, selectedWreck.Distance);
                ...
                // CHECK 3: No wrecks - fall through to orbit NPC (default behavior below)
            }

            // Default behavior: orbit the active target (NPC)
            var existingOrbitDestination2 = Core.Metatron.Movement.DestinationQueue.FirstOrDefault(
                destination => destination.ApproachType == ApproachTypes.Orbit);

            if (existingOrbitDestination2 != null)
            {
                if (distance != (int)existingOrbitDestination2.Distance)
                {
                    LogMessage(methodName, LogSeverityTypes.Debug, "Adjusting orbit on entity \'{0}\' ({1}, {2}) to {3}m.",
                        activeTarget.Name, activeTarget.ID, activeTarget.Distance, distance);
                    existingOrbitDestination2.Distance = distance;   //Update the existing orbit entry
                }
            }
            else
            {
